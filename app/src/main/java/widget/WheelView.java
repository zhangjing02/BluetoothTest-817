...
public class WheelView extends TosGallery {
	 ...
	 * Shadow colors
	 */
	private static final int[] SHADOWS_COLORS = { 0x00000000, 0x00FFFFFF, 0x00FFFFFF };
	...
	public WheelView(Context context, AttributeSet attrs) {
		super(context, attrs);
		int id = attrs.getAttributeResourceValue(null, "typeface", 0);
		if (id != 0) {
			resouceId = Integer.valueOf(context.getResources().getString(resouceId)); //��������ȡ���� typefaceName="@string/typefaceName"��ʹ��
		} else {
			if(attrs.getAttributeValue(null, "typeface")!=null){
				resouceId = Integer.valueOf(attrs.getAttributeValue(null, "typeface")); //ȡ����ֵ typefaceName="Proxima-Nova-Semibold"��ʹ��	
			}
		}
		initialize(context);
	}
	...
	private void initialize(Context context) {
		...
		if(resouceId==2){
			this.mSelectorDrawable = getContext().getResources().getDrawable(R.drawable.wheel_val2);
		}else if(resouceId==3){
			this.mSelectorDrawable = getContext().getResources().getDrawable(R.drawable.wheel_val3);	
		}else{
		...
		if(resouceId==2){
			this.setBackgroundResource(R.drawable.wheel_bg2);
		}else{
